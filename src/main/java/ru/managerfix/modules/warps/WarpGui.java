...
public class WarpGui {
    ...
    public void open(Player player, int page) {
        ...
        GuiBuilder builder = GuiBuilder.of(54)
                .title("<gradient:#FF4D00:#FAA300>Варпы сервера (" + (page + 1) + "/" + totalPages + ")</gradient>")
                .frame(true)
                .holderId("warps_list");
        ...
        if (page > 0) {
            builder.button(45, Button.builder(new ItemBuilder(Material.ARROW)
                    .name("<#FF4D00>Предыдущая страница")
                    .build())
        ...
        if (page < totalPages - 1) {
            builder.button(53, Button.builder(new ItemBuilder(Material.ARROW)
                    .name("<#FF4D00>Следующая страница")
                    .build())
                    .onClick(e -> open(player, finalPage + 1))
                    .build());
        }
    ...
    private void addWarpButton(GuiBuilder builder, Player player, Warp warp, int slot) {
        ...
        ib.name((hasPerm ? "<#FF4D00>" : "<#C0280F>") + warp.getName());

        List<String> lore = new ArrayList<>(warp.getDescription());
        lore.add("");
        lore.add("<#E0E0E0>Мир: <#FFFFFF>" + (warp.getLocation().getWorld() != null ? warp.getLocation().getWorld().getName() : "???"));
        lore.add("<#E0E0E0>Задержка: <#FFFFFF>" + warp.getTeleportDelay() + " сек.");
        if (warp.getOwner() != null) {
            String ownerName = org.bukkit.Bukkit.getOfflinePlayer(warp.getOwner()).getName();
            lore.add("<#E0E0E0>Владелец: <#FFFFFF>" + (ownerName != null ? ownerName : "???"));
        }
        lore.add("");
        if (hasPerm) {
            lore.add("<#FAA300>Нажмите, чтобы телепортироваться");
        } else {
            lore.add("<#C0280F>Нет доступа");
        }
